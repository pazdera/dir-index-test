...
scripts/create_files.py "$DIR_TYPE" "$test_dir" \
            $DIRSIZE "$FSIZE" "0" >/dev/null

for mode in $EXT4_MODES; do
    # results root for the current settings
    rroot="$RESULTS_DIR/$FS-$mode/${DIRSIZE}_files"
    mkdir -p "$rroot"
    ...
    if [ "$mode" == "spd" ]; then
        ...
        export LD_PRELOAD=`pwd`"/spd_readdir.so"
    elif [ "$mode" == "10000" ]; then
        export SPD_READDIR_CACHE_LIMIT=10000
        export LD_PRELOAD=`pwd`"/spd_readdir.so"
    elif [ "$mode" == "50000" ]; then
        export SPD_READDIR_CACHE_LIMIT=50000
        export LD_PRELOAD=`pwd`"/spd_readdir.so"
    else
        export LD_PRELOAD=""
        unset SPD_READDIR_CACHE_LIMIT
    fi
